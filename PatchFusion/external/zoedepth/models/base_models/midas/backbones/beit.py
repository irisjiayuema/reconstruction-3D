...
class AdaIn(nn.Module):
    def __init__(self, latent_size, out_channels, mean_shift=0.0, var_shift=0.0, scale=1.0):
        super(AdaIn, self).__init__()
        self.AdaINMLP = nn.Sequential(
    ...
    def forward(self, x, latent):
        style = self.AdaINMLP(latent)

        x = x.transpose(1, 2)

        device = x.device
        mean_shift = torch.tensor(self.mean_shift, dtype=x.dtype, device=device)
        var_shift = torch.tensor(self.var_shift, dtype=x.dtype, device=device)
        scale = torch.tensor(self.scale, dtype=x.dtype, device=device)
        
        shape = [-1, 2, x.size(1)] + (x.dim() - 2) * [1]
        style = style.view(shape).to(device)

        mean = style[:, 1] - mean_shift
        var = style[:, 0] + 1.0 - var_shift

        x = x * (var * scale) + mean
        x = 1.25*x.transpose(1, 2)

        return x



def forward_beit(pretrained, x, z):
    return forward_adapted_unflatten(pretrained, x, z)
...
def block_forward_adain(self, x, resolution, z = None, shared_rel_pos_bias: Optional[torch.Tensor] = None):
    """
    Use the stored noise and latent vector z for AdaIN layers.
    """
    ...
    # Use the stored noise and latent vector
    # x = x + self.adain_noise

    x = self.adain(x, z)  # Use the stored latent vector z

    return x


def forward_features_adain(self, x, z):
    ...
    for blk in self.blocks:
        if (i+1) % 4 == 0 and i != 0 and i != 23:  
            if self.grad_checkpointing and not torch.jit.is_scripting():
                x = checkpoint(blk, x, shared_rel_pos_bias=rel_pos_bias)
            else:
                x = blk(x,resolution, global_z, shared_rel_pos_bias=rel_pos_bias)
        else:
            if self.grad_checkpointing and not torch.jit.is_scripting():
                x = checkpoint(blk, x, shared_rel_pos_bias=rel_pos_bias)
            else:
...
def _make_beit_backbone(
    ...
    backbone.model.forward_features = types.MethodType(forward_features_adain, backbone.model)
    ...
    for block in backbone.model.blocks:
        ...
        if (i+1) % 4 == 0 and i != 0 and i != 23:
            block.adain = AdaIn(512,1024)
            block.forward = types.MethodType(block_forward_adain, block)
        else:
...
def _make_pretrained_beitl16_384(pretrained, use_readout="ignore", hooks=None):
    ...
    #             print(f"Source code of {attribute_name}:\n{source_code}\n")
    #         except TypeError:
    #             print(f"{attribute_name} is not a function or method, or is built-in.")
    #         except Exception as e:
    #             print(f"Could not get source code for {attribute_name}: {e}")
